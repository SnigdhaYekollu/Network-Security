...
public class PageFile {
	/*
	 * Define the page size in bytes The size of the page file will always be
	 * increments of this values
	 */
	...
	public static void main(String[] args) {
		...
		/*
		 * Create a binary table file. An actual NEW table would begin with a
		 * single page and increase in size only when the first page becomes
		 * full. However, this example demonstrates how to add/remove increments
		 * of pages to/from files after inserting only a few records into the
		 * first page.
		 *
		 * Note that whenever the length of a RandomAccessFile is increased, the
		 * added space is padded with 0x00 value bytes.
		 */
		...
		try {
			...
			/*
			 * Records are fixed size 32 bytes rowid INT - 4 bytes name CHAR(24)
			 * - 24 bytes gpa FLOAT - 4 bytes
			 */
			...
			/*
			 * Even though the first page isn't full yet... add a new blank
			 * page. Then continue inserting records at the beginning of the
			 * second page.
			 */
			...
			binaryFile.seek(
					pageLocation + recordLocation); /*
													 * Locate the pointer to the
													 * beginning of the 2nd page
													 */
			binaryFile.writeInt(4); // Write rowid=4
			binaryFile.seek(pageLocation + recordLocation + 4);
			binaryFile.writeBytes("Cathy Williams"); // Write name="Cathy
														// Williams"
			binaryFile.seek(pageLocation + recordLocation + 28);
			...
			/*
			 * Increase the size of the binaryFile by exactly one page,
			 * regardless of how long it currently is, by getting the current
			 * file length, i.e. binaryFile.length(), then adding a pageSize
			 * number of byte. The new bytes will be appended to the end and be
			 * all zeros.
			 */
			...
			/*
			 * Decrease the size of the binaryFile by exactly one page,
			 * regardless of how long it currently is, by getting the current
			 * file length, i.e. binaryFile.length(), then subtracting a
			 * pageSize number of bytes. A pageSize number of bytes will be
			 * removed from the highest address, the last page. Any data there
			 * will be lost.
			 */
			binaryFile.setLength(binaryFile.length() - pageSize);
			System.out.println("The file is now " + binaryFile.length() + " bytes long");
	 ...
	 *            <p>
	 *            This method will display the contents of the file to Stanard
	 *            Out (stdout) as hexadecimal byte values.
	 */
